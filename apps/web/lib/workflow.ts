...
// All queries run through tquery() so the RLS tenant pin (migration 0029)
// backs the explicit tenant_id predicates.
import type { Account } from "@/lib/auth";
import { calendarEventById, linkCalendarEventLead, tquery } from "@/lib/db";
...
export async function listLeads(
  ...
  const { rows } = await tquery<Lead>(tenantId,
    `${SELECT}
...
export async function bureaus(tenantId: string): Promise<string[]> {
  const { rows } = await tquery<{ bureau: string }>(tenantId,
...
export async function assignableReporters(
  ...
  const { rows } = await tquery<{ id: string; name: string }>(tenantId,
...
export async function createLead(args: {
  ...
  const { rows } = await tquery<{ id: string }>(args.tenantId,
...
export async function transition(
  ...
  const { rows } = await tquery<{ status: string; assignee_id: string | null }>(tenantId,
...
export async function assignLead(
  ...
  const { rowCount } = await tquery(tenantId,
    `update story_leads
