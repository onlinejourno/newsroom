...
//
// This guard is the belt: the explicit `where tenant_id = $1` in every query.
// The suspenders are DB-level RLS (migration 0029) — every scoped helper in
// these files routes through withTenant()/tquery(), so a query this scan
// somehow missed would return zero rows, not another newsroom's data.
// rls-live.test.ts proves that behaviour against a real database.
//
