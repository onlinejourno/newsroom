...
//
// Tenant isolation is TWO layers (ADR 0005):
//   1. The explicit `where tenant_id = $1` in every query (belt), enforced by
//      the static scan in tenant-isolation.test.ts.
//   2. Row-level security (suspenders): every tenant-scoped helper here routes
//      through withTenant()/tquery(), which pin app.current_tenant for the RLS
//      policies applied by migration 0029. A forgotten tenant_id filter now
//      returns zero rows instead of crossing tenants.
// Only the tenants-table lookups (tenantIdForSlug etc.) stay on raw pool.query
// — the tenant must be resolvable before it can be pinned.
// NOTE: RLS is enforced only for non-superuser roles; point prod DATABASE_URL
// at a role without BYPASSRLS (see 0029's header and rls-live.test.ts).
 ...
 * migration 0029 filter every tenant-scoped table by
 * current_setting('app.current_tenant'); the explicit `where tenant_id = $1`
 * in each query stays as belt-and-suspenders.
 */
...
export async function tquery<T extends QueryResultRow = QueryResultRow>(
  tenantId: string,
  text: string,
  params: unknown[] = [],
...
export async function needMixCounts(
  ...
  const { rows } = await tquery<NeedMixRow>(tenantId,
    `
...
export async function listSources(tenantId: string): Promise<SourceRow[]> {
  const { rows } = await tquery<SourceRow>(tenantId,
...
export async function deleteSource(tenantId: string, id: string): Promise<void> {
  await tquery(tenantId, "delete from sources where tenant_id = $1 and id = $2", [
    tenantId,
    id,
  ]);
}
...
export async function listConnectors(tenantId: string): Promise<ConnectorRow[]> {
  const { rows } = await tquery<ConnectorRow>(tenantId,
...
export async function deleteConnector(
  ...
  await tquery(tenantId, "delete from connectors where tenant_id = $1 and id = $2", [
    tenantId,
    id,
  ]);
}
...
export async function listSurfaces(tenantId: string): Promise<SurfaceRow[]> {
  const { rows } = await tquery<SurfaceRow>(tenantId,
...
export async function enabledSurfaceKeys(tenantId: string): Promise<string[]> {
  const { rows } = await tquery<{ key: string }>(tenantId,
...
export async function listJournalists(tenantId: string): Promise<JournalistRow[]> {
  const { rows } = await tquery<JournalistRow>(tenantId,
...
export async function signalsForJournalist(
  ...
  const { rows } = await tquery<SignalRow>(tenantId,
    `
...
export async function fetchCalendarEvents(
  ...
  const { rows } = await tquery<CalendarEventRow>(tenantId,
...
export async function fetchLatestBrief(
  ...
  const { rows } = await tquery<BriefRow>(tenantId,
    `
...
export async function listBeats(
  ...
  const { rows } = await tquery<{ slug: string; name: string }>(tenantId,
...
export async function fetchSignalUrls(
  ...
  const { rows } = await tquery<{ id: string; url: string }>(tenantId,
...
export async function fetchShortlistRanked(
  ...
  const { rows } = await tquery<ShortlistRow>(tenantId,
    `
...
export async function fetchDistributionFit(
  ...
  const { rows } = await tquery<FitScore & { story_id: string }>(tenantId,
...
export async function storiesWithScores(
  ...
  const { rows } = await tquery<ScoredStoryRow>(tenantId,
    `
...
export async function distinctSignalBeats(tenantId: string): Promise<string[]> {
  const { rows } = await tquery<{ beat: string }>(tenantId,
...
export async function entityWindows(
  ...
  const { rows } = await tquery<{ entities: string[] | null }>(tenantId,
    `
    select coalesce(
...
export async function signalsMentioning(
  ...
  const { rows } = await tquery<{ id: string; headline: string | null; url: string }>(
    tenantId,
    `
...
export async function topicDomains(
  ...
  const { rows } = await tquery<{ host: string; n: number }>(tenantId,
    `
...
export async function distinctSignalRegions(
  ...
  const { rows } = await tquery<{ region: string }>(tenantId,
...
// Regional Gaps (editor view): where signals flow vs where reporters are.
...
export async function regionCoverage(
  ...
  const { rows } = await tquery<RegionCoverageRow>(tenantId,
    `
    with sig as (
...
export async function coverageMatrix(tenantId: string): Promise<CoverageRow[]> {
  const { rows } = await tquery<CoverageRow>(tenantId,
    `
    with feeds as (
...
export async function storyClassifications(
  ...
  const { rows } = await tquery<{ frame: string | null; user_need: string | null }>(
    tenantId,
    `
...
export async function storyCount(tenantId: string): Promise<number> {
  const { rows } = await tquery<{ n: number }>(tenantId,
...
// Onboarding (3 questions, under 2 minutes): create the journalist whose
...
export async function seoAuditForUrl(
  ...
  const { rows } = await tquery<SeoAuditRow>(tenantId,
...
export async function cachedOutletKeywords(
  ...
  const { rows } = await tquery<{ keywords: unknown; computed_at: Date }>(tenantId,
...
export async function publishedPerDay(
  ...
  const { rows } = await tquery<{ d: string; n: number }>(tenantId,
    `
    with span as (
...
export async function topicInterestSeries(
  ...
  const { rows } = await tquery<TopicSeriesPoint>(tenantId,
    `
    with span as (
      select generate_series(
...
export async function outletChannelMarkers(
  ...
  const { rows } = await tquery<OutletChannelMarker>(tenantId,
    `
    select channel,
...
export async function storyClusters(
  ...
  const { rows } = await tquery<StoryCluster>(tenantId,
    `
...
export async function ownTopicStandings(
  ...
  const { rows } = await tquery<OwnStanding>(tenantId,
    `
    select t.topic,
...
export async function openLeadsRanked(tenantId: string, limit: number): Promise<TodayLead[]> {
  const { rows } = await tquery<TodayLead>(tenantId,
    `
...
export async function newsroomNowCounts(tenantId: string): Promise<NowCountsRow> {
  const { rows } = await tquery<NowCountsRow>(tenantId,
    `
    select
...
export async function navStageCounts(
  ...
  const { rows } = await tquery<NavCountsRow>(tenantId,
    `
    select
...
export async function entityCoverage(
  ...
  const { rows } = await tquery<EntityCoverageRow>(tenantId,
    `
...
export async function pitchesForEntities(
  ...
  const { rows } = await tquery<PitchedLeadRow>(tenantId,
    `
