...
def audit_page(
    ...
    *,
    ...
    # with_external=False must mean NO live calls — GDELT included — so offline
    # audits (and the default test run) stay deterministic.
    da_result = (
        gdelt_mod.domain_authority(topic, domain=domain)
        if (topic and with_external)
        else {"available": False, "score": 0.0}
    )
