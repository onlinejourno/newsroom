...
def score_aio(page: "Page", *, trend: str = "", now: datetime) -> dict:
    ...
    aio_schemas = {"NewsArticle", "FAQPage", "HowTo", "Article"}
    ...
    if any(t in aio_schemas for t in types):
        ...
        sch_pts = 10
        sch_note = f"Schema present but not AIO-optimised ({', '.join(types)}) — add NewsArticle"
    else:
        sch_pts = 0
        sch_note = "No structured data — add NewsArticle JSON-LD for AIO eligibility"
