...
def tenant_id_for_slug(conn: psycopg.Connection, slug: str) -> UUID:
    """Resolve a tenant slug to its UUID and pin the connection for RLS —
    every run resolves its tenant through here before touching scoped tables."""
    with conn.cursor() as cur:
...
def signals_for_alert(
    ...
    *,
    ...
    """High-trend signals not yet alerted (the Alert stage)."""
    with conn.cursor() as cur:
        cur.execute(
            """
